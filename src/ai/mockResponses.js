...
export async function extractFromPhoto(input) {
  ...
  const names = ['翠湖大酒店', '锦绣宾馆', '云上民宿', '春城酒店', '青石客栈']
  ...
  return {
    content: JSON.stringify({
      fields: {
        name,
        category,
        creditCode: '91530100MA' + Math.random().toString(36).substr(2, 8).toUpperCase(),
        detailAddress: '云南省昆明市五华区某路' + Math.floor(Math.random() * 200 + 1) + '号',
      },
      confidence: 0.85,
...
export async function nlQuery(question) {
  ...
  // 区域筛选
  const cityMatches = ['昆明', '曲靖', '玉溪', '保山', '昭通', '丽江', '普洱', '临沧']
  const cityCodeMap = { 昆明: '530100', 曲靖: '530300', 玉溪: '530400', 保山: '530500', 昭通: '530600', 丽江: '530700', 普洱: '530800', 临沧: '530900' }
  let cityFilter = ''
