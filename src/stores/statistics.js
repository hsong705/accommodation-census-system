...
export const useStatisticsStore = defineStore('statistics', () => {
  ...
  async function fetchDashboardData() {
    ...
    try {
      ...
      // 计算平均值
      const operatingUnits = allAccommodations.filter(a => a.operatingStatus === 'operating')
      const avgOccupancyRate = operatingUnits.length
        ? Math.round(operatingUnits.reduce((s, a) => s + (a.occupancyRate || 0), 0) / operatingUnits.length * 10) / 10
        : 0
      const avgAdr = operatingUnits.length
        ? Math.round(operatingUnits.reduce((s, a) => s + (a.adr || 0), 0) / operatingUnits.length)
        : 0
      ...
      const operatingFilter = (a) => a.operatingStatus === 'operating'
      ...
      dashboardData.value = {
        ...
        topByAdr: buildTop('adr', 5, operatingFilter),
        topByOccupancy: buildTop('occupancyRate', 5, operatingFilter),
        topByRooms: buildTop('rooms', 5),
        topByRevenue: buildTop('annualRevenue', 5, operatingFilter),
      }
    } finally {
      loading.value = false
    }
