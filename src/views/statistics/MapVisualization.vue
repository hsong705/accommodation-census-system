  ...
  <div class="page-container">
      ...
      <div class="page-header">
        ...
        <div style="display: flex; gap: 12px; align-items: center;">
          <el-radio-group v-model="metric" size="small" @change="recompute">
            ...
            <el-radio-button label="avgAdr">平均房价</el-radio-button>
            <el-radio-button label="avgOccupancy">平均入住率</el-radio-button>
          </el-radio-group>
    ...
    <el-card shadow="never" style="margin-top: 16px;">
      ...
      <el-table :data="visibleRegionStats" stripe border size="small">
        ...
        <el-table-column prop="avgOccupancy" label="平均入住率" width="120" align="center" sortable>
          <template #default="{ row }">{{ row.avgOccupancy }}%</template>
        </el-table-column>
        <el-table-column prop="avgAdr" label="平均房价" width="100" align="center" sortable>
          <template #default="{ row }">¥{{ row.avgAdr }}</template>
        </el-table-column>
...
function buildStats(list, groupBy /* 'city' | 'county' */) {
  ...
  for (const a of list) {
    ...
    if (!map[code]) {
      map[code] = {
        ...
        count: 0, rooms: 0, occupancySum: 0, adrSum: 0, operatingCount: 0,
      }
    }
    const m = map[code]
    m.count++
    ...
    if (a.operatingStatus === 'operating') {
      m.occupancySum += a.occupancyRate || 0
      m.adrSum += a.adr || 0
      m.operatingCount++
    }
  }
  return Object.values(map).map(m => ({
    ...m,
    avgOccupancy: m.operatingCount ? Math.round(m.occupancySum / m.operatingCount * 10) / 10 : 0,
    avgAdr: m.operatingCount ? Math.round(m.adrSum / m.operatingCount) : 0,
  }))
}
